...
def run_agent(tools_cfg, prompt: str) -> str:
    with ToolCollection.from_mcp(tools_cfg, trust_remote_code=True) as tool_collection:
        agent = ra.RetrievalAgent(tools=[*tool_collection.tools], model=build_model(), add_base_tools=False)
...
print("Agent returns: ", run_agent(REMOTE_MCP, "Who are the most influential users on r/politics?"))